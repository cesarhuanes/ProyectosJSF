package com.cardif.satelite.soat.multisocio.dao;
...
public interface OrdenCompraPeMapper
{
	final String SELECT_BY_PK_POLIZA_PE = "SELECT OCP.ORDEN_COMPRA, OCP.CODIGO_SOCIO, OCP.ESTADO_PAGO, OCP.FECHA, OCP.GLOSA_ESTADO_PAGO, OCP.HORA, OCP.MONTO, OCP.NUMERO_TRANSACCION, OCP.VERSION " +
			"FROM ORDEN_COMPRA_PE OCP LEFT JOIN POLIZA_PE POP ON OCP.ORDEN_COMPRA = POP.ORDEN_COMPRA WHERE POP.ID = #{pkPolizaPe,jdbcType=NUMERIC} AND OCP.CODIGO_SOCIO = #{codigoSocio,jdbcType=VARCHAR}";
	...
	public OrdenCompraPe selectByPkPolizaPeAndSocio(@Param("pkPolizaPe") Long pkPolizaPe, @Param("codigoSocio") String codigoSocio);
