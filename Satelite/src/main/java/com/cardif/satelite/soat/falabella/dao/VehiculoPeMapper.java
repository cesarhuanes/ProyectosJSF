...
public interface VehiculoPeMapper {
  ...
  final String SELECT_REG_VEHICULAR_OLD = "SELECT "
      + "(select rut_propietario from PROPIETARIO where id =(select propietario FROM direccion_propietario_vehiculo where placa = #{placa})) as propietario, "
      + "b.nombre_modelovehiculo as modelo, c.nombre_marcavehiculo as marca, A.numero_serie,a.anio_vehiculo, " + "a.uso_vehiculo, A.CATEGORIA_CLASE_VEHICULO, a.numero_asiento, "
      + "(select nombre_departamento  FROM departamento where " + "cod_departamento = (select d.departamento from direccion d, direccion_propietario_vehiculo  e "
      + "where d.id_direccion= e.direccion and e.placa = #{placa} )) as departamento " + "FROM vehiculo_pe A, modelo_vehiculo b, marca_vehiculo c "
      + "WHERE A.placa = #{placa} and a.modelo_vehiculo=b.id and b.marca_vehiculo = c.id and a.marca_vehiculo = c.id";
  ...
  final String UPDATE_BY_PLACA = "update VEHICULO_PE " + "set ANIO_VEHICULO = #{vehiculoPe.anioVehiculo}," + "NUMERO_ASIENTO = #{vehiculoPe.numeroAsiento},"
      + "NUMERO_SERIE = #{vehiculoPe.numeroSerie}," + "VERSION = #{vehiculoPe.version}," + "CATEGORIA_CLASE_VEHICULO = #{vehiculoPe.categoriaClaseVehiculo},"
      + "MARCA_VEHICULO = #{vehiculoPe.marcaVehiculo}," + "MODELO_VEHICULO = #{vehiculoPe.modeloVehiculo}," + "USO_VEHICULO = #{vehiculoPe.usoVehiculo}" + "where PLACA = #{vehiculoPe.placa}";

  @Update(UPDATE_BY_PLACA)
  int updateByPlaca(@Param("vehiculoPe") VehiculoPe vehiculoPe);
  ...
  VehiculoPe selectByPrimaryKey(BigDecimal id);
