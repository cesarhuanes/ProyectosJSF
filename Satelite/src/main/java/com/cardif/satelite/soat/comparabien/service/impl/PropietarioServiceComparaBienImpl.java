...
import com.cardif.satelite.soat.comparabien.dao.PropietarioComparaBienMapper;
...
@Service("propietarioServiceComparaBien")
public class PropietarioServiceComparaBienImpl implements PropietarioServiceComparaBien {

  public static final Logger log = Logger.getLogger(PropietarioServiceComparaBienImpl.class);
  @Autowired
  private PropietarioComparaBienMapper propietarioComparaBienMapper;
  ...
  public Propietario obtener(String dni) throws SyncconException {
    ...
    try {
      ...
      propietario = propietarioComparaBienMapper.selectPropietario(dni);
  ...
  public int insertar(Propietario propietario) throws SyncconException {
    ...
    try {
      ...
      numRowsAfected = propietarioComparaBienMapper.insert(propietario);
      if (log.isDebugEnabled())
  ...
  public boolean actualizar(Propietario propietario) throws SyncconException {
    ...
    try {
      if (log.isDebugEnabled())
        log.debug("Input [ " + BeanUtils.describe(propietario).toString() + " ]");
      propietarioComparaBienMapper.updateByPrimaryKey(propietario);
      flag = true;
      if (log.isDebugEnabled())
