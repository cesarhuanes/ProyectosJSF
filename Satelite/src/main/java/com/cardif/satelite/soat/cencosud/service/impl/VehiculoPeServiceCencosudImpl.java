...
import com.cardif.satelite.soat.cencosud.dao.VehiculoPeCencosudMapper;
...
@Service("vehiculoPeServiceCencosud")
public class VehiculoPeServiceCencosudImpl implements VehiculoPeServiceCencosud {

  public static final Logger log = Logger.getLogger(VehiculoPeServiceCencosudImpl.class);

  @Autowired
  private VehiculoPeCencosudMapper vehiculoPeCencosudMapper;
  ...
  public List<ConsultaRegVehicular> buscarPlaca(String placa) throws SyncconException {
    ...
    try {
      ...
      lista = vehiculoPeCencosudMapper.selectVehicular(placa);
  ...
  public int insertarVehiculo(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      ...
      numRowsAfected = vehiculoPeCencosudMapper.insert(vehiculoPe);
      if (log.isDebugEnabled())
  ...
  public VehiculoPe findVehiculo(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      if (log.isDebugEnabled())
        log.debug("Input [ " + BeanUtils.describe(vehiculoPe).toString() + " ]");
      vehiculoPe2 = vehiculoPeCencosudMapper.selectVehiculoByPlaca(vehiculoPe.getPlaca());
      if (log.isDebugEnabled())
  ...
  public int actualizar(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      if (log.isDebugEnabled())
        log.debug("Input [ " + BeanUtils.describe(vehiculoPe).toString() + " ]");
      numRowsAffected = vehiculoPeCencosudMapper.updateByPrimaryKey(vehiculoPe);
      if (log.isDebugEnabled())
