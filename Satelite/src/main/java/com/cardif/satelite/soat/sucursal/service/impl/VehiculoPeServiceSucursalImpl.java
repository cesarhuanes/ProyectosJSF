...
import com.cardif.satelite.soat.sucursal.dao.VehiculoPeSucursalMapper;
...
@Service("vehiculoPeServiceSucursal")
public class VehiculoPeServiceSucursalImpl implements VehiculoPeServiceSucursal {

  public static final Logger log = Logger.getLogger(VehiculoPeServiceSucursalImpl.class);

  @Autowired
  private VehiculoPeSucursalMapper vehiculoPeSucursalMapper;
  ...
  public List<ConsultaRegVehicular> buscarPlaca(String placa) throws SyncconException {
    ...
    try {
      ...
      lista = vehiculoPeSucursalMapper.selectVehicular(placa);
  ...
  public int insertarVehiculo(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      ...
      numRowsAfected = vehiculoPeSucursalMapper.insert(vehiculoPe);
      if (log.isDebugEnabled())
  ...
  public VehiculoPe findVehiculo(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      if (log.isDebugEnabled())
        log.debug("Input [ " + BeanUtils.describe(vehiculoPe).toString() + " ]");
      vehiculoPe2 = vehiculoPeSucursalMapper.selectVehiculoByPlaca(vehiculoPe.getPlaca());
      if (log.isDebugEnabled())
  ...
  public int actualizar(VehiculoPe vehiculoPe) throws SyncconException {
    ...
    try {
      if (log.isDebugEnabled())
        log.debug("Input [ " + BeanUtils.describe(vehiculoPe).toString() + " ]");
      numRowsAffected = vehiculoPeSucursalMapper.updateByPrimaryKey(vehiculoPe);
      if (log.isDebugEnabled())
